<template>
  <el-row :gutter="10">
    <el-col :xs="24" :sm="12" :md="8" :lg="6" class="context">
      <CommonCard title="总销售额" :charts="`￥${salesTotal}`">
        <!-- 默认插槽 -->
        <div class="percent">
          <span>
            周同比
          ...
          <span class="percent-value">{{
            Math.abs(salesGrowthLastMonth)
          }}</span>
          <svg
            v-if="salesGrowthLastMonth > 0"
            t="1628558608250"
            class="icon"
          ...
          <span class="text">{{ Math.abs(salesGrowthLastDay) }} </span>
          <svg
            v-if="salesGrowthLastDay > 0"
            t="1628558608250"
            class="icon"
            viewBox="0 0 1024 1024"
            version="1.1"
    ...
    <el-col :xs="24" :sm="12" :md="8" :lg="6" class="context">
      <CommonCard title="访问量" :charts="`${visitTotal}`">
    ...
    <el-col :xs="24" :sm="12" :md="8" :lg="6" class="context">
      <CommonCard title="支付笔数" :charts="`${payTotal}`">
      ...
      <CommonCard title="运营活动效果" charts="78%">
        ...
        <template #footer>
          <div class="percent">
            ...
            <span class="subtext">
              日同比<span class="text">23.45% </span>
            </span>
            <svg
              t="1628558630690"
              class="icon"
...
export default {
  ...
  computed: {
    ...mapState({
      salesTotal: state => state.dashboard.chartsData.salesTotal,
      salesToday: state => state.dashboard.chartsData.salesToday,
      salesGrowthLastDay: state =>
        state.dashboard.chartsData.salesGrowthLastDay,
      salesGrowthLastMonth: state =>
        state.dashboard.chartsData.salesGrowthLastMonth,

      visitTotal: state => state.dashboard.chartsData.visitTotal,
      visitToday: state => state.dashboard.chartsData.visitToday,
      visitTrend: state => state.dashboard.chartsData.visitTrend,

      payTotal: state => state.dashboard.chartsData.payTotal,
      payRate: state => state.dashboard.chartsData.payRate,
      payTrend: state => state.dashboard.chartsData.payTrend,

      activityRate: state => state.dashboard.chartsData.activityRate,
      activityGrowthLastDay: state =>
        state.dashboard.chartsData.activityGrowthLastDay,
      activityGrowthLastMonth: state =>
        state.dashboard.chartsData.activityGrowthLastMonth
    })
  }
}
</script>
